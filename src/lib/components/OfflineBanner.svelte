...
{#if visible}
	<div class="fixed inset-x-0 top-0 z-30 px-3 pt-2" style="pointer-events: none;">
		<div class="mx-auto max-w-[480px]">
			{#if status.authExpired}
				<div role="alert" class="flex" style="pointer-events: auto;">
					<a
						href="/login?next=/"
			...
			{:else}
				<div
					...
					style="background: {status.online
						...
						: 'var(--color-line-2)'}; color: {status.online
						? 'var(--color-amber)'
						: 'var(--color-text-dim)'}; backdrop-filter: blur(6px); pointer-events: auto;"
					role="status"
				>
					<span
